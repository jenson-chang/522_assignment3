...
@Python and the following Python packages were used to perform the analysis: @pandas, @altair, @click, as well as @Allaire_Quarto_2022. *Note: this report is adapted from @ttimbers-horses.*
...
```{python}
province = horses_sd_table.loc[horses_sd_table['Std'].idxmax(), 'Province']
```

In @fig-horse_pops_plot_largest_sd we zoom in and look at the province of `{python} province`, which had the largest spread of values in terms of standard deviation.
